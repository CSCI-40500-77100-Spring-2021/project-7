...
const Chart = (ticker) => {
  ...
  useEffect(() => {
    const fetchStockData = async () => {
      const result = await getDailyChartForSymbol(ticker.ticker);
      //   console.log("chart ticker.ticker", ticker.ticker);
      const test = JSON.stringify(result);
      if (test.indexOf("Error Message") < 1) {
        setStockData(formatStockData(result.data["Time Series (Daily)"]));
      }
      console.log("chart object", result);
    };
